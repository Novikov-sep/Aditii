...
window.onload = function () {
   ...
   function documentActions(e) {
      ...
      if (window.innerWidth > 768 && isMobile.any()) {
         if (targetElement.classList.contains('header-menu__arrow')) {
      ...
      if (targetElement.classList.contains('header__search-mobile')) {
         document.querySelector('.header__search').classList.toggle('_active');
      }
      if (targetElement.classList.contains('info-products__btn')) {
      ...
      if (targetElement.classList.contains('cart-header__icon') || targetElement.closest('.cart-header__icon')) {
         ...
         e.preventDefault();
      } else if (!targetElement.closest('.cart-header') && !targetElement.classList.contains('info-products__btn')) {
...
function addToCart(productButton, productId) {
   if (!productButton.classList.contains('_hold')) {
      ...
      const productImage = product.querySelector('.products__img');
...
function updateCart(productButton, productId, productAdd = true) {
   ...
   if (productAdd) {
      ...
      if (!cartProduct) {
         const product = document.querySelector(`[data-pid="${productId}"]`);
         const cartProductImage = product.querySelector('.products__img').innerHTML;
         const cartProductTitle = product.querySelector('.products__sub-title').innerHTML;
...
if (spollersArray.length > 0) {
   ...
   //Иницилизация обычных спойллеров
   if (spollersRegular.length > 0) {
      initSpollers(spollersRegular);
   }

   //получение спойлеров с медиом запросом
   const spollersMedia = Array.from(spollersArray).filter(function (item, index, self) {
      return item.dataset.spollers.split(",")[0];
   });

   //Иницилизация спойллеров с медиа запросами
   if (spollersMedia.length > 0) {
      const breakpointsArray = [];
      spollersMedia.forEach(item => {
         const params = item.dataset.spollers;
         const breakpoint = {};
         const paramsAray = params.split(",");
         breakpoint.value = paramsAray[0];
         breakpoint.type = paramsAray[1] ? paramsAray[1].trim() : "max";
      ...
      mediaQuaries.forEach(breakpoint => {
         const paramsAray = breakpoint.split(",");
         const mediaBreakpoint = paramsAray[1];
         const mediaType = paramsAray[2];
         const matchMedia = window.matchMedia(paramsAray[0]);
